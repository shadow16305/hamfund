...
const About = () => {
  ...
  return (
    <section className="overflow-x-hidden" id="about">
      ...
      <motion.div
        style={{ x: xCard }}
        className="flex justify-center flex-col lg:flex-row mx-auto gap-x-16 gap-y-4 lg:max-w-[1320px]">
        {aboutText.map((item) => (
          <p key={item.id} className="rounded-xl text-white max-w-[340px] lg:max-w-[700px] mx-auto">
            {item.text}
          </p>
        ))}
      </motion.div>
